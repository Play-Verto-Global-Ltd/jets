module Jets::Commands::Call
  class Caller
    ...
    def remote_run
      ...
      result = resp.payload # already been normalized/JSON.dump by AWS
      unless @options[:mute_output] && result.nil?
        STDOUT.puts result.read # only thing that goes to stdout
      end
    end
